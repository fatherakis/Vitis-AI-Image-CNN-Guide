...
### Model & Dataset
First and foremost, at least **60 - 100GB** are required for the docker container and the Vitis-AI repo.
...
1. The model loaded and modified with any conversions required.
2. The dataset ready in order to run inference for the calibration step of the quantization.
3. Any further modifications in normalization, image sizing etc.

**Note**: The accuracy estimation of VitisAI does not reflect its actual performance whereas its resource estimations are spot on.

[<ins>resnet20_minimal_vai</ins>](resources/resnet20_minimal_vai.py) provides an example of the bare minimum requirements for the quantization and compilation to run smoothly.

### Running the script
As observed in the parser of the files, each one can be run with quantization mode: float, calib and test. We skip float mode in this guide as it simply runs inference on the model as every other tool.
...
**NOTE**: If an error occurs stating any problems accessing the model or finding "ModelName.py" simply rerun the command and everything is fine.
...
In order to quantize and calibrate our model we run:\
...
In order to deploy our quantized model in its .xmodel form we run:\
...
After running these commands, a "quantize_result" folder should be created containing the VAI .py version of the model and the exported .xmodel file along with some other ones for compatibility.
...
In [Board_files](resources/board/) we can find all the required files for inference on our VCK190 Board for a ResNet20 on CIFAR10.\
...
3. Dataset in the form of images and folders (dataset.tar)
...
Do not add the whole dataset. Only the images you intend to test. In its current form the image name contains the correct label and its number as label_xxx.png. This can be changed to your preference by modifying [check_runtime_top5_cifar10.py](resources/board/model_src/code/src/check_runtime_top5_cifar10.py).
...
* [cifar10_performance.sh](resources/board/model_src/cifar10_performance.sh) runs performance evaluations on the model for 1 to 6 DPU threads. There aren't much to change here except the number of threads and how the results are saved
...
``run_src.sh vck190 my_model.xmodel``
